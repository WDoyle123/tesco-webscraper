...
def scrape_page(driver, unwanted_texts):
    ...
    for container in product_containers:
        ...
        if product_name_elements and price_elements and image_elements:
            ...
            product_name = re.sub(r'[^\w\s]', '', product_name, flags=re.UNICODE)
            price = price_elements[0].text
            # Filter out invalid image URLs
            valid_image_sources = [img.get_attribute('src') for img in image_elements if img.get_attribute('src') and not img.get_attribute('src').startswith('data:image')]
...
def download_image(url):
    try:
        ...
        filename = f'../data/images/product_{product_index}.png'
        product_index += 1
...
def scraper():
    ...
    urls = ["https://www.tesco.com/groceries/en-GB/shop/fresh-food/all"]
    ...
    for base_url in urls:
        ...
        while True:
            ...
            print(len(all_products), len(all_prices), len(all_images))
        ...
        df = pd.read_parquet('../data/tesco_scrape.parquet.gzip')
